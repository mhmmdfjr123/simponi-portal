...
@section('content')
    <ol class="breadcrumb page-breadcrumb">
        <!-- Auto Breadcrumbs -->
        <li class="active" data-active-menu="#menu-file-download"><a href="#">Ubah File Download</a></li>
    </ol>
    ...
    <form action="{{ url('backoffice/file/download/submit') }}" method="post" class="validate" enctype="multipart/form-data">
        ...
        <input type="hidden" name="old_file_name" value="{{ $obj->file_name }}" />
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" maxlength="50" class="form-control required" value="{{ $obj->name }}" />
                        </div>
                        <div class="form-group">
                            <label class="control-label">Deskripsi</label>
                            <textarea name="desc" rows="4" maxlength="300" class="form-control required">{{ $obj->desc }}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="download_category_id" class="form-control required">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {!! $obj->download_category_id == $category->id ? 'selected="selected"' : '' !!}>{{ $category->name }} {!! $category->status != 'Y' ? '(Tidak Aktif)' : '' !!}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <div>
                                <a class="btn btn-default" href="{{ asset('file/storage').'/'.$obj->file_name }}" target="_blank"><i class="fa fa-download"></i> Download</a>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label">Ganti File</label>
                            <label id="px-file-custom-1" class="custom-file px-file" for="px-file-custom-input-1">
                                <input id="px-file-custom-input-1" class="custom-file-input" type="file" name="file">
